...
export default function FullTestSimulator({onExit}){
 const[mode,setMode]=useState(null),[questions,setQuestions]=useState([]),[answers,setAnswers]=useState({}),[index,setIndex]=useState(0),[remaining,setRemaining]=useState(0),[finished,setFinished]=useState(false),[review,setReview]=useState(false),[confirmOpen,setConfirmOpen]=useState(false)
 const question=questions[index]
 const start=selectedMode=>{setMode(selectedMode);setQuestions(createTest(selectedMode));setAnswers({});setIndex(0);setRemaining(CONFIG[selectedMode].seconds);setFinished(false);setReview(false);setConfirmOpen(false)}
 const handleSubmitTest=()=>setConfirmOpen(true)
 const confirmSubmit=()=>{setConfirmOpen(false);setFinished(true)}
 ...
 const answeredCount=Object.keys(answers).filter(key=>Number(key)>=0&&Number(key)<questions.length).length
 return <main className="simulator-page">{confirmOpen&&<div className="submit-confirm-backdrop" role="dialog" aria-modal="true" aria-labelledby="submit-confirm-title"><section className="submit-confirm-modal"><span>🚩</span><h2 id="submit-confirm-title">Xác nhận nộp bài</h2><p>Bạn đã hoàn thành <strong>{answeredCount}/{questions.length}</strong> câu. Bạn có chắc chắn muốn nộp bài để tính điểm không?</p><div><button type="button" onClick={()=>setConfirmOpen(false)}>Làm tiếp</button><button type="button" className="confirm-submit-button" onClick={confirmSubmit}><Flag/> Xác nhận nộp bài</button></div></section></div>}<header className="test-running"><button onClick={onExit}><ArrowLeft/> Thoát</button><div><span>{CONFIG[mode].name} · Câu {index+1}/{questions.length}</span><i><em style={{width:`${(index+1)/questions.length*100}%`}}/></i></div><strong className={remaining<300?'urgent':''}><Clock3/>{String(Math.floor(remaining/60)).padStart(2,'0')}:{String(remaining%60).padStart(2,'0')}</strong><button type="button" className="header-submit-button" onClick={handleSubmitTest}><Flag/> Nộp bài</button></header><section className="exam-question">{listeningParts.has(question.part)?<ListeningQuestion question={question} groupEntries={groupEntries} answers={{...answers,current:selected,currentIndex:index}} onAnswer={answerListening}/>:<><span>{question.part.toUpperCase()}</span>{question.passage&&<pre>{question.passage}</pre>}<h1>{question.question}</h1><div className="exam-options">{question.options.map((option,i)=><button className={selected===i?'selected':''} onClick={()=>setAnswers(old=>({...old,[index]:i}))} key={i}><b>{String.fromCharCode(65+i)}</b><span>{option}</span></button>)}</div></>}<footer className="exam-bottom-bar"><button disabled={index===0} onClick={()=>setIndex(previousIndex)}><ArrowLeft/> Trước</button><div className="exam-navigation-actions">{!atEnd&&<button type="button" className="early-submit-button" onClick={handleSubmitTest}>Nộp bài sớm</button>}<button type="button" className={atEnd?'final-submit-button':'next-question-button'} onClick={()=>atEnd?handleSubmitTest():setIndex(nextIndex)}>{atEnd?'🏁 Nộp bài & Xem điểm':'Tiếp'} {!atEnd&&<ArrowRight/>}</button></div></footer></section></main>
}
